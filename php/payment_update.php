...
$signed= hash_hmac('sha256', $signing, 'S-f-YyFqBBiTBLGRsvPhikhg');
if ($signed === $data['x_signature']) {
    if ($paidstatus == "Success"){ //payment success
        $sqlinsertpayment = "INSERT INTO `tbl_orders`( `user_email`,`payment_id`, `order_paid`,`order_status`) VALUES ('$email','$paymentid','$amount','$status')";
        $sqpupdatecart = "UPDATE `tbl_cart` SET `cart_status`='paid',`payment_id`='$paymentid' WHERE user_email='$email' AND cart_status IS NULL";
        if ($conn->query($sqlinsertpayment) && $conn->query($sqpupdatecart)){
            $sqlselectcart="SELECT * FROM tbl_cart WHERE payment_id = '$paymentid'";
            $result = $conn->query($sqlselectcart);
            if ($result->num_rows > 0) {
                while ($rows = $result->fetch_assoc()) {
                    $sjid = $rows['product_id'];
                    $sjqty = $rows['cart_qty'];
                    $updateproductsqty = "UPDATE tbl_subjects set subject_sessions = (subject_sessions-$sjqty) WHERE subject_id = '$sjid'";
